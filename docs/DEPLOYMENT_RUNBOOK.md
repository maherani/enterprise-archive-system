...
این سند راهنمای جامع و مرجع عملیاتی نصب، پیکربندی، استقرار از صفر (روی سرور خام) و نگهداری سامانه آرشیو اسناد سازمانی است. تمامی گام‌ها بر اساس آخرین وضعیت مخزن گیت (`main`)، ماژول بومی `archive_autotag v1.8.0`، فیلتر پیشرفته چندتگی و مکانیزم‌های حاکمیت داده تدوین شده‌اند.
...
اسکریپت `deploy/deploy_from_scratch.sh` تمامی مراحل راه‌اندازی، انتظار برای سلامت پایگاه‌داده، نصب بدون نیاز به مداخله (Headless) نکست‌کلود، کپی و فعال‌سازی ماژول بومی `archive_autotag v1.8.0`، تنظیم پراکسی معتمد و بررسی سلامت را خودکار انجام می‌دهد:
...
## ۵. تگ‌گذاری خودکار داینامیک و فیلتر همپوشانی چندتگی (`archive_autotag v1.8.0`)
...
## ۹. گردش‌کار درخواست ایجاد پوشه توسط ادمین گروه و تأیید ادمین کل (Delegated Folder Creation Workflow v1.8.0)
